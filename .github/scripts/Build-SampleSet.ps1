...
$exclusionReasons = @{}
Import-Csv '.\.github\resources\exclusions.csv' | ForEach-Object {
    ...
    foreach ($configuration in $Configurations) {
        foreach ($platform in $Platforms) {
            ...
            if ($exclusionConfigurations.ContainsKey($sampleName) -and ($exclusionConfigurations[$sampleName].Split(';') | Where-Object { "$configuration|$platform" -like $_ })) {
                ...
                .\.github\scripts\Build-Sample -Directory $directory -SampleName $sampleName -LogFilesDirectory $LogFilesDirectory -Configuration $configuration -Platform $platform -InfVerif_AdditionalOptions $InfVerif_AdditionalOptions -Verbose:$Verbose
